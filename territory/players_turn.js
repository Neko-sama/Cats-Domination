...
function mostrarInfo(){
  ...
  var textenemy="";
  var texttime="";
  if(hexOverObject.owner==0){
    textowner="El terreno no tiene dueño.";
  }else{
    var textarmy;
    if(hexOverObject.base==false){
      if(hexOverObject.army==0){
        textarmy="El terreno no tene ejercito.";
      }else{
        textarmy="Tiene "+hexOverObject.army+" ejercitos.";
      }
      textowner="Pertenece a jugador "+(hexOverObject.owner) + "\n"+textarmy;
    }else{
      textowner="Pertenece a jugador "+(hexOverObject.owner)+"\nEste Terreno tiene una base\n \nDefensa restante: "+hexOverObject.defense;
    }
  }

  hexowner.text = textowner;
  if(hexOverObject.terrainType==0){
      texttype="Normal";
  }else if (hexOverObject.terrainType==1) {
    texttype="Recurso"+"\nGenera "+ hexOverObject.resoursePerTurn+" recursos.";
    //poner cantidad de recuersos
  }else{
    texttype="Túnel";
  }
  if(hexOverObject.enemy>0){
    textenemy="¡Cuidado, hay un lobo aquí!";
  }
  hextype.text = "\nTipo: "+texttype + "\n"+textenemy;
...
/*hexOverTile.owner = 0
hexOverTile.terrainType = type;
hexOverTile.base = false;
hexOverTile.defense = 0;
hexOverTile.army = 0;
*/
  if(turn==1){
    hexowner.cameraOffset.setTo(panel.offset*2+20,350);
    hextype.cameraOffset.setTo(panel.offset*2+20, 350+20);
    timeturntext.cameraOffset.setTo(panel.offset*2+20, 350+80);
    ...
    timeturntext.cameraOffset.setTo(screenWidth-170, 350+80);
  }
  if(state==0){
...
function EndGame(game,winnerplayer){
  ...
  if(winnerplayer!=0)
    widthtext=200
  var mensaje = game.add.text(0,0, "  Fin del juego \n¡Victoria del Jugador "+winnerplayer+"!", { font: "40px "+panel.fontName, fill: panel.fontColor, align: 'center' });
  if(winnerplayer==0)
    mensaje.text="Fin del Juego \n¡Empate!";
  //gametimer.stop();
  gamemusic = territory.game.add.audio('gameover');
  gamemusic.play();
  mensaje.stroke = "#000000";
  mensaje.strokeThickness = 2;
  mensaje.setShadow(2, 2, "#000000", 2, true, false);
  mensaje.fixedToCamera = true;
  mensaje.cameraOffset.setTo(screenWidth/2- widthtext,screenHeight/2-150);
  game.time.events.remove(gametimer);
  updateStatistics(winnerplayer);
  }

function updateStatistics(winnerplayer){
  switch (winnerplayer) {
    case 1:
    localStorage.setItem('Persa.wins',+localStorage.getItem('Persa.wins')+1);
    localStorage.setItem('Siames.defeats',+localStorage.getItem('Siames.defeats')+1);
      break;
    case 2:
    localStorage.setItem('Persa.defeats',+localStorage.getItem('Persa.defeats')+1);
    localStorage.setItem('Siames.wins',+localStorage.getItem('Siames.wins')+1);
      break;
    case 0:
    localStorage.setItem('Persa.draws',+localStorage.getItem('Persa.draws')+1);
    localStorage.setItem('Siames.draws',+localStorage.getItem('Siames.draws')+1);
      break;
    default:

  }
  localStorage.setItem('Game.plays',+localStorage.getItem('Game.plays')+1);
}
...
function createPanel(game,hexagon){
  ...
  timeturntext= game.add.text(panel.offset*2+20, 350+90, 'Tiempo de turno \nrestante: ', { font: "30 px "+panel.fontName, fill: panel.fontColor, align: "center" });
  ...
  button_newbase_red.setDownSound(buttonsound);
  button_endturn_red.setDownSound(buttonsound);
  ...
  button_newbase_blue.setDownSound(buttonsound);
  button_endturn_blue.setDownSound(buttonsound);
  //donde va ala info
  ...
  timeturntext.stroke = "#000000";
  timeturntext.strokeThickness = 2;
  timeturntext.setShadow(2, 2, "#000000", 2, true, false);
  //territorios1.stroke = "#000000";
  //territorios1.strokeThickness = 1;
  //territorios1.setShadow(1, 1, "#000000", 2, true, false);
  //recursos1.stroke = "#000000";
  ...
  timeturntext.fixedToCamera = true;
  hexowner.visible=false;
  hextype.visible=false;
  timeturntext.visible=false;
  //territorios1.fixedToCamera = true;
  //recursos1.fixedToCamera = true;
  //army1.fixedToCamera = true;
  //marcos
...
function updatePanel(game){
  ...
  timeturntext.text = "Tiempo de turno \nrestante:  "+timeturn;
...
//sistema de turnos
  ...
  button_newbase_red.visible=false;
  button_endturn_red.visible=false;
  //button_move_blue.visible=true;
  //button_attack_blue.visible=true;
  cat2.visible=true;
  raza2.visible=true;
  hab2.visible=true;
  bases2.visible=true;
